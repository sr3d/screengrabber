...
| [`TextRecognizer.swift`](../Sources/screengrabber/TextRecognizer.swift) | On-device OCR via `VNRecognizeTextRequest` (Vision framework — no entitlement, no network). Recognizes on a background queue and sorts the observations into reading order, since Vision returns them unordered. | ~65 |
| [`LiveTextOverlay.swift`](../Sources/screengrabber/LiveTextOverlay.swift) | Preview-style text selection on the image, wrapping VisionKit's `ImageAnalysisOverlayView`. Sits between that overlay and `CanvasView` and decides per click which gets the event — in automatic mode it claims only points over recognized text with no annotation on top, so the drawing tools are unaffected. Two traps documented inline: `analysisHasText(at:)` takes **top-left** origin points although the view reports `isFlipped == false`, and analysis is debounced because every stroke invalidates it. | ~215 |
| [`ExtractedTextWindowController.swift`](../Sources/screengrabber/ExtractedTextWindowController.swift) | The floating **Extracted Text** panel behind the toolbar's **OCR** button (⇧⌘T): an editable text view plus Copy All (which copies the edited text, so OCR slips can be fixed in place). Runs against `CanvasView.compositeImage()`, so it follows crops and respects blur/box redactions rather than recovering text the user hid. | ~150 |
